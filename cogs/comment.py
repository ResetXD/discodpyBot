...
class Phcomment(commands.Cog):
    ...
    async def ph(self, ctx:Context,who:discord.Member = None,*,text=None):
        if who == None:
        ...
        try:
            ...
            d = await c.get_imagegen_phcomment(imgType="phcomment",image=b,text=text,username=who)
...
def setup(client):
    client.add_cog(Phcomment(client))